...
//                can_free_echo_skb(dev->netdev, frn - 1, NULL);
                ...
                if (frn == expectedfrn) {
                        ...
                        dev->frn_read++;
                        result = 1;
                        netdev_dbg(dev->netdev, "can_get_echo_skb %d", frn - 1, NULL);
...
//                                can_free_echo_skb(dev->netdev, frn - 1, NULL);
                        ...
                        while (expectedfrn != frn) {
//                                can_free_echo_skb(dev->netdev, expectedfrn - 1, NULL);
                                expectedfrn++;
                        }
...
static int ixxat_pci_probe(struct pci_dev *dev, const struct pci_device_id *id)
{
        ...
        if (intf->dmavadd)
                dma_free_coherent(dev == NULL ? NULL : &dev->dev, intf->dmalen, intf->dmavadd, intf->dmaadd);
